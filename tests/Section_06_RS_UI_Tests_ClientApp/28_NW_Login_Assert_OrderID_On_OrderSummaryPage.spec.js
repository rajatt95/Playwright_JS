...
test('Section_06_RS_UI_Tests- RahulShettyAcademy Client App Login - Assertion for Order ID Order Summary Page', async ({page} )=> {
    ...
    const orderID_order_summary_page = await page.locator('.col-text').textContent();
    console.log('Assertion for Order ID on Order Summary page');
    expect(orderID.includes(orderID_order_summary_page)).toBeTruthy();
